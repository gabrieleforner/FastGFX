...
namespace FastGFX::Resource {
    Shader::Shader(const Core::Renderer& renderer) : m_renderer(&renderer){}
    Shader Shader::useInternalShader(std::string shader_name)
    {
        for (auto& shader : engineShaders)
        {
            ...
            {
                this->useExternalVS(shader.vsFile);
                this->useExternalFS(shader.fsFile);
    ...
    Shader Shader::Create()
    {
        ...
        attributes[1].offset = offsetof(Vertex, rgbaColor);
        ...
        inputSetting.vertexAttributeDescriptionCount = static_cast<uint32_t>(attributes.size());;
        inputSetting.vertexBindingDescriptionCount = 1;
        inputSetting.pVertexAttributeDescriptions = attributes.data();
        ...
        iaConfiguration.primitiveRestartEnable = VK_TRUE;
        ...
        vkDestroyShaderModule(this->m_renderer->engineDevice, this->vertex, nullptr);
        vkDestroyShaderModule(this->m_renderer->engineDevice, this->frag, nullptr);
    ...
    void Shader::Bind() {
        ...
        vkCmdDraw(this->m_renderer->commandBuffer, 3, 1, 0, 0);
    }
    void Shader::Destroy() {
